...
<script>
// When opening edit modal, populate image inputs
function openProductEditModalPopulateImages(imagesJson) {
  const imgs = imagesJson ? (Array.isArray(imagesJson) ? imagesJson : JSON.parse(imagesJson)) : [];
  ...
  imgs.forEach(url => addProductEditImageInput(url));
}

// Hook into existing open function if present
if (typeof openProductEditModal === 'function') {
  const orig = openProductEditModal;
  openProductEditModal = function(id) {
    orig(id);
    // after modal opens, fetch product details and populate images
    fetch(`/api/admin/products/${id}`).then(r => r.json()).then(json => {
      if (!json.success) return;
      openProductEditModalPopulateImages(json.data.images ? JSON.parse(json.data.images) : []);
    }).catch(() => {});
  };
}
</script>
